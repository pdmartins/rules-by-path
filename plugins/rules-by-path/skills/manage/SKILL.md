...
5. **Create it** — body on stdin:
   ...
   the rule inherits the distance its type declares. `--exclude` and `--tool`
   narrow the rule further — see *Narrowing further* below.
...
`list` shows each rule's globs and, in brackets, the filters that narrow them.
`validate` reports rules that can never fire (no glob, or an `exclude` that
takes back everything the glob covers), empty rules, and notes long rules,
shared globs, names outside the type convention, an unreadable `tool:` value,
the pre-0.4.0 frontmatter key, and a total that exceeds one injection's budget. `config`
